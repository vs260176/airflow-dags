# Импортируем класс datetime из стандартной библиотеки Python для работы с датами и временем.
from datetime import datetime
# Импортируем декораторы dag и task из airflow.decorators. Это современный способ определения DAG и задач.
from airflow.decorators import dag, task

# ЭТОТ ИМПОРТ УДАЛЕН ИЗ ВЕРХНЕГО УРОВНЯ:
# from airflow.providers.postgres.operators.postgres import PostgresOperator 
# Закомментированная строка, которая изначально могла присутствовать, но была удалена. 
# Это указывает на изменение подхода: вместо использования операторов верхнего уровня (Top-Level Operators) 
# для задач используются Python-функции с декоратором @task и Хуки (Hooks) внутри них.

# Определяем константу для хранения идентификатора подключения к базе данных Postgres.
# Этот идентификатор должен быть предварительно настроен в веб-интерфейсе Airflow.
...
@dag(
    ...
    # Дата начала выполнения DAG. Airflow начнет планировать запуски с этого момента.
    start_date=datetime(2023, 11, 1),
    # Расписание выполнения. '@hourly' означает запуск каждый час.
    schedule='@hourly',
    # Параметр catchup=False отключает "наверстывание" пропущенных запусков между start_date и текущим моментом.
    catchup=False,
)
# Определяем функцию, которая содержит логику всего DAG. Название функции совпадает с dag_id.
def process_hourly_db_partition_dag_v2():
    ...
    def create_summary_table_task():
        ...
        # Ленивый импорт: модуль загружается в память только при вызове функции (во время выполнения задачи).
        from airflow.providers.postgres.hooks.postgres import PostgresHook
        # Создаем экземпляр PostgresHook, используя заданный ранее connection ID.
    ...
    # **kwargs используется для доступа к контекстным переменным Airflow (например, data_interval_start).
    @task
    def aggregate_and_insert_data(**kwargs):
        ...
        # Получаем время начала интервала выполнения DAG из контекста Airflow и форматируем его в строку SQL.
        ds_start = kwargs['data_interval_start'].strftime('%Y-%m-%d %H:%M:%S')
        # Получаем время окончания интервала выполнения DAG из контекста Airflow и форматируем его в строку SQL.
    ...
    # Определяем зависимости между задачами с помощью оператора >> (task_1 >> task_2),
    # что означает: task_1 должна успешно завершиться перед запуском task_2.
    create_summary_table_task() >> aggregate_and_insert_data()

# Вызываем функцию-фабрику DAG, чтобы зарегистрировать его в глобальном пространстве имен Airflow.
