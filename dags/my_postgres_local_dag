...
# Указываем Airflow, какой Connection ID использовать по умолчанию для PostgresOperator
...
def process_hourly_db_partition_dag():
    ...
    # 1. Задача: Создать целевую таблицу, если ее нет (можно вынести в отдельный dag)
